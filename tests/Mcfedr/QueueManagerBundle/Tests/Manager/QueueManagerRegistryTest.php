...
final class QueueManagerRegistryTest extends TestCase
{
    ...
    public function testDelete(): void
    {
        ...
        $this->default->expects($this->once())->method('delete')->with($job);
        $this->delay->expects($this->never())->method('delete');
    ...
    {
        $job = $this->getMockBuilder(Job::class)->getMock();
        $this->default->expects($this->once())->method('delete')->with($job)->willThrowException(new WrongJobException());
        $this->delay->expects($this->once())->method('delete')->with($job);
    ...
    public function testDeleteNoMatch(): void
    {
        $job = $this->getMockBuilder(Job::class)->getMock();
        $this->default->expects($this->once())->method('delete')->with($job)->willThrowException(new WrongJobException());
        $this->delay->expects($this->once())->method('delete')->with($job)->willThrowException(new WrongJobException());
    ...
    {
        $this->default->expects($this->once())->method('put')->with('name', ['args'], ['options']);
        $this->delay->expects($this->never())->method('put');
    ...
    {
        $this->default->expects($this->never())->method('put');
        $this->delay->expects($this->once())->method('put')->with('name', ['args'], ['options']);
